...
function render(d) {
  ...
  /* user cover image(s) — front photo + optional back photo(s) */
  const photoEl = document.getElementById('photo-el');
  if (photoEl) {
    const photos = [];
    if (d.photo)  photos.push(d.photo);
    
    // Support multiple background photos from photos2, fallback to photo2
    if (Array.isArray(d.photos2) && d.photos2.length > 0) {
      photos.push(...d.photos2);
    } else if (d.photo2) {
      photos.push(d.photo2);
    }
  ...
  const acts = [];
  if (d.mobile) {
    const mobiles = Array.isArray(d.mobile) ? d.mobile : [d.mobile];
    mobiles.forEach((num, index) => {
      if (num) {
        acts.push({ 
          href: `tel:${num}`, 
          ...
          lbl: mobiles.length === 1 ? (isAr ? 'اتصال' : 'Call') : `${isAr ? 'اتصال' : 'Call'} ${index + 1}` 
        });
      }
    });
  }
  ...
  if (d.whatsapp) {
    const whatsapps = Array.isArray(d.whatsapp) ? d.whatsapp : [d.whatsapp];
    whatsapps.forEach((num, index) => {
      if (num) {
        const waNum = formatEgyptianWhatsApp(num);
        acts.push({ 
          ...
          lbl: whatsapps.length === 1 ? 'WhatsApp' : `WhatsApp ${index + 1}`, 
          target: '_blank' 
        });
      }
    });
  ...
  if (d.publicEmail) {
    const emails = Array.isArray(d.publicEmail) ? d.publicEmail : [d.publicEmail];
    emails.forEach((email, index) => {
      if (email) {
        acts.push({ 
          ...
          lbl: emails.length === 1 ? (isAr ? 'إيميل' : 'Email') : `${isAr ? 'إيميل' : 'Email'} ${index + 1}` 
        });
      }
    });
  }
  ...
  if (d.location) {
    ...
    locations.forEach((loc, index) => {
      ...
      infoRows.push({
        ...
        lbl: locations.length === 1 ? (isAr ? 'الموقع' : 'Location') : `${isAr ? 'الموقع' : 'Location'} ${index + 1}`, 
        val: locVal, 
        target: '_blank'
      });
    });
  ...
  if (d.website) {
    ...
    websites.forEach((web, index) => {
      ...
      infoRows.push({ 
        ...
        lbl: websites.length === 1 ? (isAr ? 'الموقع الإلكتروني' : 'Website') : `${isAr ? 'الموقع الإلكتروني' : 'Website'} ${index + 1}`, 
        val: clean, 
        target: '_blank' 
      });
    });
      ...
      val.forEach((url, index) => {
        if (url) {
          activeSocials.push({
            key: p.key,
            icon: p.icon,
            label: index === 0 ? p.label : `${p.label} ${index + 1}`,
            url: url
          });
        }
      });
  ...
  if (socialCard && socialGrid) {
    if (activeSocials.length) {
      ...
      socialGrid.innerHTML = activeSocials.map(p =>
        `<a href="${p.url}" class="soc-btn" target="_blank" rel="noopener">` +
...
function downloadVCard() {
  ...
  if (d.mobile) {
    const arr = Array.isArray(d.mobile) ? d.mobile : [d.mobile];
    arr.forEach(val => { if (val) lines.push(`TEL;TYPE=CELL:${val}`); });
  }
  if (d.publicEmail) {
    const arr = Array.isArray(d.publicEmail) ? d.publicEmail : [d.publicEmail];
    arr.forEach(val => { if (val) lines.push(`EMAIL:${val}`); });
  }
  if (d.website) {
    const arr = Array.isArray(d.website) ? d.website : [d.website];
    arr.forEach(val => { if (val) lines.push(`URL:${val}`); });
  }
  if (d.location) {
    const arr = Array.isArray(d.location) ? d.location : [d.location];
    arr.forEach(val => { if (val) lines.push(`ADR:;;${val};;;;`); });
  }
  if (d.photo) {
    lines.push(`PHOTO;VALUE=URI:${d.photo}`);
  }
  if (d.bio) {
    lines.push(`NOTE:${d.bio}`);
  }
