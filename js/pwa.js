...
(function() {
  ...
  window.addEventListener('appinstalled', () => {
    ...
    hideInstallTriggers();
    showToast('🎉 تم تثبيت تطبيق أوردر بنجاح! تجده الآن في شاشة هاتفك وقائمة التطبيقات كبرنامج مستقل.');
  });
  ...
  function renderInstallBanner() {
    ...
    const appName = document.title.includes('إدارة') ? 'إدارة أوردر' : 'أوردر';
    ...
    banner.innerHTML = `
      <div style="position:relative; width:44px; height:44px; border-radius:11px; overflow:hidden; flex-shrink:0; background:#120e0c; border:1px solid rgba(255,255,255,0.1);">
        <img src="icons/icon-192.png" alt="App Icon" style="width:100%; height:100%; object-fit:cover;">
      </div>
      <div style="flex:1; min-width:0;">
        <div style="font-size:13.5px; font-weight:800; color:var(--text-main, #fff); line-height:1.3;">تثبيت تطبيق ${appName}</div>
        <div style="font-size:11.5px; color:var(--text-muted, #a8a29e); margin-top:2px;">تجربة سريعة بدون متصفح وبدون علامة كروم</div>
      </div>
  ...
  window.triggerPWAInstall = async function() {
    if (isStandalone) {
      showToast('🚀 التطبيق مثبت بالفعل ويعمل كبرنامج مستقل!');
      return;
    }
        ...
        <div style="background:var(--surface, #1c1714); border:1px solid var(--border-strong, #ea580c); border-radius:20px; padding:22px; max-width:92vw; width:380px; box-shadow:0 20px 40px rgba(0,0,0,0.6); color:var(--text-main, #fff);">
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:14px;">
            <div style="display:flex; align-items:center; gap:10px;">
              <img src="icons/icon-192.png" style="width:38px; height:38px; border-radius:10px;">
              <div style="font-weight:800; font-size:15px;">تثبيت أوردر على iPhone / iPad</div>
            </div>
  ...
  function showFallbackInstallModal() {
    ...
    if (!modal) {
      ...
      modal.innerHTML = `
        <div style="background:var(--surface, #1c1714); border:1px solid var(--border-strong, #ea580c); border-radius:20px; padding:24px; max-width:92vw; width:400px; box-shadow:0 20px 40px rgba(0,0,0,0.6); color:var(--text-main, #fff); text-align:center;">
          <div style="width:54px; height:54px; border-radius:14px; margin:0 auto 12px; overflow:hidden; background:#120e0c;">
            <img src="icons/icon-192.png" style="width:100%; height:100%; object-fit:cover;">
          </div>
          <div style="font-weight:800; font-size:16px; margin-bottom:6px;">تثبيت التطبيق على جهازك</div>
          <p style="font-size:12.5px; color:var(--text-muted, #aaa); margin-bottom:16px; line-height:1.6;">
            لتثبيت التطبيق كبرنامج مستقل بدون علامة المتصفح:
            <br>
            من قائمة المتصفح <b>(الـ 3 نقاط ⋮ أعلى الشاشة)</b>، اختر <b>'تثبيت التطبيق' (Install app)</b>.
          </p>
          <button onclick="document.getElementById('fallback-pwa-modal').remove()" style="background:linear-gradient(135deg, #ea580c, #f97316); color:#fff; border:none; padding:10px 24px; border-radius:12px; font-weight:800; font-size:13.5px; cursor:pointer;">حسناً، فهمت</button>
        </div>
      `;
      modal.addEventListener('click', (e) => {
        if (e.target === modal) modal.remove();
